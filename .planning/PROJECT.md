...
A personal tech blog running on a self-hosted Kubernetes cluster with full observability integration. The blog features MDX-based content, project showcases, and article listings. Deployed via ArgoCD with GitHub Actions CI/CD pushing multi-arch images to GHCR. Includes server-side page analytics with Prometheus metrics and a GitOps-provisioned Grafana dashboard.

## Core Value

The blog runs reliably on the home Kubernetes cluster with the same performance as Vercel, while providing full observability (logs, traces, metrics, RUM, page analytics) through the existing stack.
...
- Page view metrics per path exposed at /metrics (stateless counters) — v1.1
- Referrer tracking with source attribution (Google, Twitter, etc.) — v1.1
- User agent parsing into browser/device categories — v1.1
- Grafana dashboard combining Prometheus metrics + Faro sessions — v1.1
- Dashboard deployed via Helm chart ConfigMap (GitOps) — v1.1
...
- Vercel Analytics — replaced with self-hosted observability
...
**Current State:**
- Shipped v1.1 with 3,311 LOC TypeScript
- Tech stack: Next.js 15, React 19, Tailwind CSS v4, TypeScript
- Container: node:22-alpine, 381MB image with Sharp
- Observability: OTEL traces, Prometheus metrics, Faro RUM, Grafana dashboard
- Analytics: Page views, traffic sources, browser/device detection
- Deployment: ArgoCD syncing from Git, GHCR container registry

**Target Environment:**
- Home Kubernetes cluster with existing infrastructure
- ArgoCD for GitOps deployments
- Observability stack: Alloy, Tempo, Loki, Prometheus, Faro, Grafana
...
| Header-passing for Edge-to-Node metrics | prom-client requires Node.js, middleware runs Edge | ✓ Good |
| isbot for bot detection | Reliable, well-maintained, comprehensive | ✓ Good |
| bowser for UA parsing | Handles Chromium variants correctly | ✓ Good |
| Domain-to-source mapping | Individual platform tracking (google, twitter) | ✓ Good |
| UTM validation (50-char limit) | Prevents cardinality explosion | ✓ Good |
