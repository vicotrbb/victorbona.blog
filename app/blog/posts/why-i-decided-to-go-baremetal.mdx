...
I went bare metal because I wanted to understand the full production path: hardware, operating system, networking, storage, Kubernetes, GitOps, observability, backups, and cost. The point is not rejecting managed cloud; the point is making future cloud choices from knowledge instead of dependency.

## Key takeaways

- Bare metal turns infrastructure from an opaque service into a learning system.
- Owning the layers makes managed services an intentional tradeoff instead of a reflex.
- NixOS, K3s, ArgoCD, Longhorn, MinIO, and observability are the core pieces of this setup.
- The real payoff is operational confidence: debugging, recovery, cost visibility, and architectural judgment.
