...
My current homelab is a small home data center built around pfSense, a managed switch, two Proxmox nodes, TrueNAS, Proxmox Backup Server, media services, observability, infrastructure services, and a few AI experiments. The main lesson is that network control, storage separation, and backups matter more than adding another random self-hosted app.

## Key takeaways

- pfSense and a managed switch moved the network from consumer-router chaos to a controlled edge.
- Proxmox separates container-heavy daily services from warm VM and experiment capacity.
- TrueNAS and Proxmox Backup Server give storage and recovery their own machines.
- Observability is useful even at home because it turns "why is this slow?" into a real investigation.
- The best homelab improvements are usually boring: DNS, backups, VLANs, power, and documentation.
...
### Wi-Fi: ASUS mesh and a myriad of smart stuff
